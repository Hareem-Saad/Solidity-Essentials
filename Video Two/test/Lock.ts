  ...
  describe("change owner", function () {
    describe("Validations", function () {
      it("Should revert with the right error if not called by owner", async function () {
        ...
        await expect(lock.connect(otherAccount).changeOwner(otherAccount.address)).to.be.revertedWith(
          "user not authorized"
        );
      });
    });
    ...
    describe("Transfer of Ownership", function () {
      it("Should change the owner to the set address", async function () {
        const { lock, unlockTime, lockedAmount, owner, otherAccount } = await loadFixture(
          deployOneYearLockFixture
        );
